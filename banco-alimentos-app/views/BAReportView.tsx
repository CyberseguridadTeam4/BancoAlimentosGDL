...
export default function BAReportView({
    closeSheet,
    type,
    objId
}: ReportProps){
    ...
    const reason = ['No me gusta', 'Abuso y acoso', 'Discurso violento', 'Spam', 'Estafa o fraude', 'Información Falsa'];
        ...
        <View style={styles.container}>
            {reason.map((reason) => (
                <View key={reason} style={styles.item}>
                    ...
                    thumbColor={selectedReason === reason ? BAPallete.White: BAPallete.SoftRed}
...
const ConfirmReportModal = ({reason, closeSheet, type, objId}: any) => {
    ...
    if(reason == null){
        return (
            <View>
                <BAText>Debes seleccionar una razón por la cual realizas el reporte</BAText>
            </View>
        );
    } else {
        return (
